...
class OutputManager:
    ...
    def __init__(self, max_hosts=25):
...
╚═══════════════════════════════════════════════════════════════════╝{Colors.RESET}
    ...
    def display_category_overview(self, categorized_hosts):
        ...
        for category in category_order:
            if category in categorized_hosts and categorized_hosts[category]:
                ...
                print(
                    f"  {name:<25} {Colors.GREEN}{count:>4}{Colors.RESET} ({percentage:5.1f}%) - {display_mode}"
                )
    ...
    def _display_small_category(self, category, hosts):
        ...
        print(f"\n{color_code}{'─'*70}")
        print(f"{name} ({len(hosts)} hosts - Showing ALL)")
        print(f"{'─'*70}{Colors.RESET}")
    ...
    def _display_large_category(self, category, hosts):
        ...
        color_code = getattr(Colors, color, Colors.WHITE)
        print(f"\n{color_code}{'─'*70}")
        print(f"{name} ({len(hosts)} hosts - Showing top {self.max_hosts})")
        print(f"{'─'*70}{Colors.RESET}")

        self.print_warning(
            f"Large category! Showing {self.max_hosts} of {len(hosts)} hosts."
        )
    ...
    def _display_host(self, host, index, color):
        ...
        print(
            f"  {Colors.DIM}{index:3}.{Colors.RESET} "
    ...
    def _get_category_config(self, category):
        ...
        configs = {
            ...
            "https_only": {"name": "HTTPS ONLY", "color": "BLUE"},
            "http_only": {"name": "HTTP ONLY", "color": "YELLOW"},
            "redirects": {"name": "REDIRECTING", "color": "MAGENTA"},
            "errors": {"name": "SERVER ERRORS", "color": "RED"},
            "blocked": {"name": "ACCESS BLOCKED", "color": "RED"},
            "no_response": {
                "name": "NO RESPONSE",
                "color": "BRIGHT_YELLOW",
            },
        }
        return configs.get(category, {"name": category.upper(), "color": "WHITE"})
    ...
    def display_quick_stats(self, stats):
        ...
        # Category stats
        stats_table2.append(
            ("Fully Operational", str(stats.get("fully_operational", 0)))
        )
        ...
        for (label1, value1), (label2, value2) in zip(stats_table1, stats_table2):
            print(
                f"{Colors.CYAN}{label1:<{col_width}}{Colors.RESET}{Colors.GREEN}{value1:>10}{Colors.RESET}   "
            ...
            if hosts:
                ...
                if len(hosts) <= 10:
                    ...
                    for host in hosts[:8]:
                        print(f"  * {host}")
                    print(
                        f"  {Colors.BRIGHT_YELLOW}... and {len(hosts) - 8} more{Colors.RESET}"
                    )
...
class ReconCategorizer:
    ...
    def categorize_results(results):
        ...
        for result in results:
            # httpx uses 'status-code' not 'status_code'
            status = result.get("status-code", 0)
            url = result.get("url", "")

            # Some versions of httpx might use 'input' field
            if not url and "input" in result:
                url = result["input"]
            ...
            # Get title - httpx uses 'title' field
            ...
            if status == 0:  # No response
                ...
                if status == 200:
                    ...
                    if is_https:
                        stats["https_only"] += 1
                        categorized["https_only"].append(host_data)
                    else:
                        stats["http_only"] += 1
                        categorized["http_only"].append(host_data)
                ...
                # Unknown status code - treat as operational
                stats["dns_resolved"] += 1
                stats["online_hosts"] += 1
                stats["web_services"] += 1
                stats["fully_operational"] += 1
...
def run_subfinder(domain, output_file):
    ...
    print(
        f"{Colors.BLUE}[*]{Colors.RESET} Running Subfinder for: {Colors.CYAN}{domain}{Colors.RESET}"
    )
    ...
    try:
        ...
        if os.path.exists(output_file):
            with open(output_file, "r") as f:
                ...
                if subdomains:
                    print(
                        f"{Colors.GREEN}[+]{Colors.RESET} Found {Colors.CYAN}{len(subdomains)}{Colors.RESET} subdomains"
                    )
                    return True
        return False
...
def run_httpx(input_file, output_file, simple_mode=False):
    ...
    print(
        f"{Colors.BLUE}[*]{Colors.RESET} Running httpx on {Colors.CYAN}{len(subdomain_list.splitlines())}{Colors.RESET} subdomains..."
    )

    # Build httpx command - use -sc for status code
    if simple_mode:
        cmd = ["httpx", "-silent", "-sc", "-title", "-json"]
    else:
        cmd = ["httpx", "-silent", "-sc", "-title", "-td", "-json"]
    ...
    try:
        ...
        stdout, stderr = process.communicate(input=subdomain_list, timeout=900)
            ...
            if lines:
                with open(output_file, "w") as f:
                    for line in lines:
                        f.write(line + "\n")
                print(
                    f"{Colors.GREEN}[+]{Colors.RESET} Found {Colors.GREEN}{len(lines)}{Colors.RESET} alive hosts"
                )
                return True
        else:
            print(f"{Colors.YELLOW}[!]{Colors.RESET} httpx returned no output")
            if stderr:
                print(f"{Colors.YELLOW}[!] stderr: {stderr[:200]}{Colors.RESET}")
            return False

    except subprocess.TimeoutExpired:
        print(f"{Colors.RED}[-]{Colors.RESET} httpx timed out after 900 seconds")
        return False
    except Exception as e:
        print(f"{Colors.RED}[-]{Colors.RESET} httpx error: {str(e)}")
        return False
        ...
        for line in f:
            ...
            if line:
                try:
                    ...
                    # Check if httpx returned valid data
                    if "url" in data or "input" in data:
                        # Normalize the data
                        if "status-code" not in data:
                            data["status-code"] = 0
                    ...
                    print(
                        f"{Colors.YELLOW}[!] JSON parse error: {str(e)}{Colors.RESET}"
                    )
                    print(
                        f"{Colors.YELLOW}[!] Problematic line: {line[:100]}{Colors.RESET}"
                    )
                    continue
...
def save_enhanced_results(results, categorized, stats, interesting, domain, output_dir):
    ...
    try:
        ...
        print(
            f"{Colors.GREEN}[+]{Colors.RESET} JSON report: {Colors.CYAN}{json_file}{Colors.RESET}"
        )
    except Exception as e:
    ...
    try:
        ...
        print(
            f"{Colors.GREEN}[+]{Colors.RESET} Text report: {Colors.CYAN}{txt_file}{Colors.RESET}"
        )
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Recon Tool v4.0 - Subfinder + httpx Automation",
...
{Colors.CYAN}Examples for Large Results:{Colors.RESET}
  ...
  {sys.argv[0]} -d example.com --simple          # Simple mode (no tech detection)
    ...
    parser.add_argument(
        "--max-hosts", type=str, default="25", help="Max hosts per category"
    )
    ...
    if args.max_hosts.lower() == "all":
        ...
        try:
            ...
            print(
                f"{Colors.RED}[-] Invalid --max-hosts value. Use number or 'all'{Colors.RESET}"
            )
            sys.exit(1)

    # Initialize output manager
    output = OutputManager(max_hosts=max_hosts)
    output.print_banner()
    ...
    print(
        f"{Colors.BLUE}[*]{Colors.RESET} Target: {Colors.CYAN}{args.domain}{Colors.RESET}"
    )
    print(
        f"{Colors.BLUE}[*]{Colors.RESET} Start: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print(
        f"{Colors.BLUE}[*]{Colors.RESET} Max hosts per category: {max_hosts if max_hosts < 999999 else 'all'}"
    )
    print()
    ...
    try:
        ...
        print(
            f"{Colors.GREEN}[+]{Colors.RESET} Successfully processed {len(results)} results"
        )
        print()
        ...
        save_enhanced_results(
            results, categorized, stats, interesting, args.domain, args.output
        )
